@unless($recent->isEmpty())
    <div class="my-4">
        ...
        <div class="flex flex-col max-w-sm px-4 py-6 mx-auto bg-white dark:bg-gray-800 rounded-[2rem] ltr:rounded-br-none rtl:rounded-bl-none shadow-sm">
            @foreach($recent as $post)
                <a href="{{ route('post',$post->slug) }}" class="border-b border-t border-white hover:border-primary-600 transition duration-300 px-1 py-4">
                    <div class="flex items-center space-x-3 rtl:space-x-reverse">
                        @if($post->image() !== null)
                            <img alt="{{ $post->title }}" src="{{ $post->image() }}" class="h-6 w-6 shadow-sm rounded-[2rem] rounded-bl-none z-0 object-cover"/>
                        @endif
